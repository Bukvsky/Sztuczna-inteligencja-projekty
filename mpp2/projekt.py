...
def askUser():
    ...
    print(f'''1.Wyświetl wynik klasyfikacji algorytmu delta dla perceptronu dla wag bazowych''')
    ...
    print(f'''3.Podaj swoje wlasne atrybuty do zaklasyfikowania dla wag bazowych''')
    print(f'''4.Podaj swoje wlasne atrybuty do zaklasyfikowania dla wag wlasnych''')
    print(f'''5.Exit''')
    print(f'''{'=' *122}''')
...
def main():
    ...
    while True:
        ...
        if(choice == 1):
            ...
            try:
                user_weights = input(f"Podaj wagi  (oddzielonne przecinkami)")
            ...
            try:
                ...
                if len(inputs) != l:
                    print(f"Błąd: oczekiwano {l} atrybutów")
                    continue
                prediction = user_perceptron.compute(inputs)
                print(f"Klasa: Iris-setosa" if prediction == 1 else "Inna klasa (versicolor/virginica)")
            except ValueError:
                print(f"Błąd: nieprawidłowy format danych")
        ...
        """            try:
                user_weights = input(f"Podaj wektor atrybutów (oddzielonych przecinkami, np. '5.1,3.5,1.4,0.2')")
                inputs = list(map(float, user_weights.split(',')))
                user_threshold = float(input(f"Podaj próg: "))
                user_learning_rate = float(input(f"Podaj współczynnik uczenia: "))
                if len(inputs) != l:
